...
export const getUserBolts = async (address) => {
    ...
    if (address) {
        ...
        campaignPoints = await getUserPointsByCampaign(address)
    }
    ...
    return {
        ...
        galxe,
        zealy,
        fuul: { user, leaderboard, campaignPoints },
    }
}
...
const getUserPointsByCampaign = async (address) => {
    const url = `https://api.fuul.xyz/api/v1/payouts?type=point&user_address=${address.toLowerCase()}`
    const HEADERS = {
