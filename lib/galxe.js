...
export const updateGalxePointsForAllUsers = async (campaignId) => {
    const campaignIds = ["GCHpqttjuU", "GCJGCth5pr", "GCQxktdaW3", "GCQxktdaW3", "GCNdVtdWfe"]
    ...
    console.log(`Galxe usersWithNewPoints: ${usersWithNewPoints.length}. Unique users: ${aggregatedUsers.length}`)
    ...
    return { updated: usersWithNewPoints.length, unique: aggregatedUsers.length }
}
    ...
    for (const user of users) {
        ...
        if (!existingUser) {
            await prisma.user.create({
                data: { id: user.address.toLowerCase(), galxePoints: user.points }
            })
