...
export function getNativeCoinInfo(network) {
  ...
  const NATIVE_COIN_NAMES = {
    ...
    ARBITRUM_SEPOLIA: "Arbitrum Sepolia ETH",
    SEPOLIA: "Sepolia ETH",
    AVALANCHE: "AVAX",
...
export const formatDataNumber = (input, decimals = 18, formatDecimal = 2, currency, compact) => {

  let res = BigNumber.from(input).div(BigNumber.from(10).pow(decimals));

  let value = res.toNumber();
