...
export const updateTokenPrice = async (network, symbol, price, execute, channelId) => {
  try {
    const geb = initGeb(network);
    const tokenList = Object.values(geb.tokenList).filter((token) =>
      AVAILABLE_TOKENS.includes(token.symbol)
    ...
    let oracleForTestnetAddress;
    try {
      oracleForTestnetAddress = await denominatedOracle.priceSource();
    } catch (e) {
      throw {
        message: {
          error: `oracleForTestnet not found for ${token.symbol}`,
          bytes32String: token.bytes32String,
          delayedOracle: delayedOracleAddress,
          denominatedOracle: denominatedOracleAddress,
...
Price Change **${beforePrice}** ${execute ? `➡️ **${afterPrice}**` : ""}`,
      ...
      {
        name: `DelayedOracle ${delayedOracleSymbol} ${shouldUpdate ? "🟢 Update Ready" : "❄️ Cooling"
          }`,
