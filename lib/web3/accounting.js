...
export const accounting = async (network, channelId) => {
  let popDebtFromQueueResponse;
  // TODO: additional arguments needed for pop debt
  ...
  const fields = [
    {
      ...
      value: popDebtFromQueueResponse?.hash
        ? `[receipt](${getExplorerBaseUrlFromName(network)}tx/${popDebtFromQueueResponse?.hash
        })`
        : "Failed",
    },
    {
      ...
      value: auctionDebtResponse?.hash
        ? `[receipt](${getExplorerBaseUrlFromName(network)}tx/${auctionDebtResponse.hash
        })`
        : "Failed",
    },
    {
      ...
      value: auctionSurplusResponse?.hash
        ? `[receipt](${getExplorerBaseUrlFromName(network)}tx/${auctionSurplusResponse.hash
        })`
        : "Failed",
    },
    {
      ...
      value: transferExtraSurplusResponse?.hash
        ? `[receipt](${getExplorerBaseUrlFromName(network)}tx/${transferExtraSurplusResponse.hash
        })`
        : "Failed",
    },
  ];
